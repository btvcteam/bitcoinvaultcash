...
#if defined(NDEBUG)
# error "Bitcoinvaultcash cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x4f832a6fb2f17b3791d3522bb00bcbe4f6d1450c5e3757e185cc7a5fb5602353");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Bitcoinvaultcash: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Bitcoinvaultcash Signed Message:\n";
...
{
    // Bitcoinvaultcash: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Bitcoinvaultcash: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Bitcoinvaultcash: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Bitcoinvaultcash: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Bitcoinvaultcash: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Bitcoinvaultcash: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x49152d29bf06afa9d7a2f3172f724ef454b3777ebe52e09b9cbb604ada03fe27");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Cavaaaaaaloooo...";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04a27fe98aba7cb2f07f610ff27a24b26dbd17e28ba431f7523c11732ca60bdb8e760fed4fbe1b4885c69b0cfcabe6f243cc9ac2391a750214ef69b975a120bdd3") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1548979200;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085734732;

        if (fTestNet)
        {
            block.nTime    = 1546300800;
            block.nNonce   = 386944218;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xb1955f6c8092a505b8c26905824dacaba77379c60a8b65479d908808d0a78814"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc3, 0xb9, 0xde }; // Bitcoinvaultcash: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BitcoinvaultcashMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BitcoinvaultcashMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BitcoinvaultcashMiner(CWallet *pwallet)
{
    printf("BitcoinvaultcashMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bitcoinvaultcash-miner");
    ...
    try { loop {
        ...
        printf("Running BitcoinvaultcashMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BitcoinvaultcashMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BitcoinvaultcashMiner, pwallet));
}
